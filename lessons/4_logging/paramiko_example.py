    ...
    def connect(self):
        if not self.ssh:
            try:
                ...
                self.ssh.connect(self.ip, port=22, username=self.login, password=self.agent_password)
            ...
            if timeout > 0:
                ...
                while True:
                    ...
                    if errors:  # error on socket occurs
                        stderr += "SSH CLIENT: SOCKET ERROR"
                        break
...
ssh = BaseSSH("192.168.0.102", "tishyk", 'adelaida')
ssh.connect()
ssh.call("echo 'hello from ssh object' | systemd-cat")
output = ssh.check_output("ls -l /home/tishyk")
output = ssh.check_output("rm -rf /home/tishyk/demo")
output = ssh.check_output("rm -rf /var/log/my_app")
# output = ssh.check_output("mkdir demo")
print(output)
# ssh.send_package('app_logging.py', "/home/tishyk/demo/app_logging.py")